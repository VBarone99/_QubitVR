...
/// <summary>
/// Quantum gate child class. Performs a 180 degree rotation about the X-axis.
/// </summary>
public class XGate : Gate
{
    ...
    public XGate(int qubits, int targetIndex)
    {
        ...
        {
            Debug.LogError("Index provided to constructor XGate(int qubits, int targetIndex) is out of range. Index of " + targetIndex +
    ...
    /// <returns>Completed Nth root XGate object</returns>
    ...
    /// <param name="n">The root degree to take of the current gate matrix.
    /// n=2 -> square root of the current gate matrix. n=3 -> cube root of the current gate matrix. etc.</param>
    /// <returns>Nth root matrix of the XGate</returns>
    protected override Matrix<Complex> NthRootMatrix(int n)
    {
        ...
        for (int currentIndex = 1; currentIndex < m_numQubits; currentIndex++)
        {
            if (currentIndex == m_targetIndex)
                NthRootMatrixInSystem = NthRootMatrixInSystem.KroneckerProduct(NthRootMatrix); //pauliX
            else
