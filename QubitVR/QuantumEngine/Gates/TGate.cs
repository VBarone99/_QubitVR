...
/// <summary>
/// Quantum gate child class. Performs a 45 degree rotation about the Z-axis.
/// </summary>
public class TGate : Gate
{
    /// <summary>
    /// The qubit index in which the TGate will be applied to.
    /// </summary>
    public int m_targetIndex { get; }

    /// <summary>
    /// Constructs a TGate given the number of qubits and a qubit index to apply it to.
    /// </summary>
    /// <param name="qubits">Number of qubits in the system.</param>
    /// <param name="targetIndex">Qubit index to apply the TGate to.</param>
    public TGate(int qubits, int targetIndex)
    {
        ...
        {
            Debug.LogError("Index provided to constructor TGate(int qubits, int targetIndex) is out of range. Index of " + targetIndex +
    ...
    /// <returns>Completed Nth root TGate object</returns>
    ...
    /// <param name="n">The root degree to take of the current gate matrix.
    /// n=2 -> square root of the current gate matrix. n=3 -> cube root of the current gate matrix. etc.</param>
    /// <returns>Nth root matrix of the TGate</returns>
    protected override Matrix<Complex> NthRootMatrix(int n)
    {
        ...
        // =========================
        double theta = -(Math.PI / 4); // Potentially make positive to rotate the other way?
